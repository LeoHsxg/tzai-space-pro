...
> **後記（2026-07-20，使用者本人澄清，取代本節前提）**：「交棒給之後的齋長」不成立。
> 使用者定位：**這整個專案是他的個人練習作品**，服務對象只是一個宿舍的幾十人；
> 只要「空間借用」核心功能持續運作，其他附加功能（留言板等）壞了無所謂。
> **不需預設交接情境**——除非未來有學弟主動聯繫要求交接，否則就讓系統持續運作。
> 因此技術取捨權重修正為：**使用者的學習價值是正當且可以優先的考量**，
> 「可維護性 > 優雅 > 功能豐富」降為一般工程品味而非最高原則；評估方案時不要再以
> 「兩年後沒人維護」為由否決使用者想學的技術（實例：留言板選 Cloudflare Workers 而非
> Firebase，正是出於學習動機，經使用者拍板）。本節其餘紅線（秘密、production 資料、
> client 禁寫 Firestore）不受此後記影響。
