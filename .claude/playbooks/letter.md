...
### 1. 這個系統的壽命比它的維護者長——選無聊的解法

使用者是即將畢業的學生（見 `docs/yuchenplans/`，他自述半年後畢業），系統將交棒給
之後的齋長。這改變所有技術取捨的權重：**可維護性 > 優雅 > 功能豐富**。
具體判準：兩個方案效果相近時，選依賴少、magic 少、Firebase console 上看得懂的那個；
新增依賴前先問「兩年後沒人維護時它會不會變成負債」；不要主動提議引入新框架或架構重寫。
